...
class scatter_plot:
    ...
    def add_plot(
        ...
        if scatter:
            self.ax.scatter(x,y,c = scatter_color, label = label,s=scatter_size, marker = scatter_marker)
        if plot_line:
            if weight == None:
                self.ax.plot(x,y,linewidth=1.5, c=line_color, linestyle=line_type)
            else:
                ...
                self.ax.plot(i,wb+w*i,linewidth=1.5, c=line_color, linestyle=line_type, label = label)
                #
        if equal_aspect:
